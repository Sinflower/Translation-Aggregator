...
char *AtlasTransSJIS(char *jis, int flags)
{
	...
	char *temp = jis;
	if (!(flags & ATLAS_CAN_MODIFY)) temp = (char*)malloc(1+strlen(jis));
	int p1 = 0;
	int p2 = 0;
	int lastJis = 1;
	while (jis[p1])
	{
		...
		else
		{
			...
			if (lastJis)
				while (p2 > 0 && temp[p2-1] == ' ') p2--;
			unsigned char u = jis[p1];
			// double-byte JIS.
			if ((u >= 0x81 && u <= 0x9F) ||
				(u >= 0xE0 && u <= 0xEF)) temp[p2++] = jis[p1++];
			temp[p2++] = jis[p1++];
		}
	}
	temp[p2] = 0;
	...
	if (!(flags & ATLAS_CAN_MODIFY)) free(temp);

	if (unsure)
		FreeAtlasData(unsure,0,0,0);
	if ((flags & ATLAS_NO_FREE)) return outjis;
	if (outjis)
	{
...
int LoadAtlasDlls()
{
	if (atlecont && awdict && awuenv) return 1;
...
int InitAtlas(AtlasConfig &cfg, int transDirection)
{
	...
	if (atlecont && awdict &&
		...
		(AwuWordDel = (AwuWordDelType*) GetProcAddress(awdict, "AwuWordDel")))
		{
			...
			if (ruleSet = LoadRuleSet(path, name, cfg.flags))
			{
...
wchar_t *TranslateFull(wchar_t *otext, int lineBreakFlags, int freeText, int NeedAbort(int line, int lines, void *data), void *data)
{
	if (!otext || !ruleSet) return 0;
	int memLen = wcslen(otext);
	int len = memLen;
	int count;
	wchar_t *snipped = ruleSet->ParseText(otext, &len, &count);
	wchar_t *text = otext;
	if (!freeText)
...
#pragma omp parallel for schedule(dynamic,1)
	...
	for (int i=0; i<count; i++)
	{
		if (NeedAbort && NeedAbort(i, count, data)) break;
...
char *TranslateFull(char *otext, int lineBreakFlags, int freeText, int NeedAbort(int line, int lines, void *data), void *data)
{
	int len = -1;
	wchar_t *d = ToWideChar(otext, 932, len);
	if (!d) return 0;
	wchar_t *outw = TranslateFull(d, lineBreakFlags, freeText, NeedAbort, data);
	free(d);
	if (!outw) return 0;
	len = -1;
	char *out = ToMultiByte(outw, 932, len);
	free(outw);
	return out;
}
